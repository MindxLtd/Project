...
//function for search icon on type for search box 

function showclearicontype() {
	
	    $("#tablesearch").keyup(function(){
                    $("#searchclear").toggle(Boolean($(this).val()));
                });
                $("#searchclear").toggle(Boolean($("#tablesearch").val()));
...
function DeleteCurrent(e) {
	...
	var $el = $(e.currentTarget);
        var $row = $el.closest('tr');
		 var rowi = $row.index() + 1;
	quickdelpopup(rowi);
	return false;
//    var todelete = r.parentNode.parentNode.rowIndex;
//    var table = document.getElementById("global-table").tBodies[0];
	
//    ShowDeletePopup()
    //table.deleteRow(todelete);
}
function DeleteCurrent2t(e) {
   
	var $el = $(e.currentTarget);
	var ctable = $el.closest('table').attr("id");
        var $row = $el.closest('tr');
		 var rowi = $row.index() + 1;
	quickdelpopup2t(ctable,rowi);
	return false;
//    var todelete = r.parentNode.parentNode.rowIndex;
//    var table = document.getElementById("global-table").tBodies[0];
...
function DeleteCurrentic(e) {
   
	var $el = $(e.currentTarget);
        var $row = $el.closest('tr');
		 var rowi = $row.index() + 1;
	quickdelpopupic(rowi);
	return false;

}
function DeletesingleCurrency(e) {
...
function DeleteSingle(rowid){

	document.getElementById("global-table").deleteRow(rowid);
	var popup = document.getElementById('globalpopup');
    popup.style.display = 'none';
}
function DeleteSingle2t(t,rowid){
	var tabid = "\"" + t + "\"";
	//alert(tabid);
	document.getElementById(t).deleteRow(rowid);
	var popup = document.getElementById('globalpopup');
    popup.style.display = 'none';
}
...
function novalidfilePopup() {
    var btn = '<input type="button" value="Close" class="global-popup__cancel" onclick="closePopupBox()">';
    var t = 'Please select the file to upload';
    var h = 'No files selected';
    showPopupBox(h,t,btn);
}
...
function quickdelpopup(elem) {
    ...
    var h = 'Delete row no '+id+' ?';
    showPopupBox(h,t,btn);
}

function quickdelpopup2t(t,elem) {
	
    var btn = '<input type="button" value="Delete" class="global-popup__delete" onclick="DeleteSingle2t(\''+t+'\','+elem+')"><input type="button" value="Cancel" class="global-popup__cancel" onclick="closePopupBox()">';
    var id = elem;
    var t = 'Are you sure you would like to permanently delete this item?';
    var h = 'Delete row no '+id+' ?';
    showPopupBox(h,t,btn);
...
function quickdelpopupic(elem) {
    var btn = '<input type="button" value="Delete" class="global-popup__delete" onclick="DeleteSingleic('+elem+')"><input type="button" value="Cancel" class="global-popup__cancel" onclick="closePopupBox()">';
    var id = elem;
    var t = 'Are you sure you would like to permanently delete this item?';
    var h = 'Delete this item ?';
    showPopupBox(h,t,btn);
}

function deleteSinglePopup(elem) {
    var btn = '<input type="button" value="Delete" class="global-popup__delete" onclick="DeleteSelected()"><input type="button" value="Cancel" class="global-popup__cancel" onclick="closePopupBox()">';
    var id = elem.parentNode.parentNode.parentNode.getElementsByTagName('td')[0].innerHTML;
    var t = 'Are you sure you would like to permanently delete this item?';
    var h = 'Delete row no '+id+' ?';
    showPopupBox(h,t,btn);
}
...
function pushCurrentRow(button) {
    ...
    sessionStorage.setItem('contentlist', JSON.stringify(content));
//    location.href = 'languages_view.html';
    if (button == button.parentNode.childNodes[0]) {
        location.href='languages_view.html';
    }
    else if (button == button.parentNode.childNodes[1]) {
        location.href='languages_edit.html';
    }
    else if (button == button.parentNode.childNodes[2]) {
